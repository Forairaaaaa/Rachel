...
namespace sn
{
    ...
    bool Cartridge::loadFromFile(std::string path)
    {
        ...
        //     return false;
        // }
        for (int i = 0; i < 0x4000 * banks; i++)
            m_PRG_ROM[i] = romFile[i + 0x10];
        ...
        if (vbanks)
        {
            ...
            //     return false;
            // }
            for (int i = 0; i < 0x2000 * vbanks; i++)
                m_CHR_ROM[i] = romFile[i + 0x10 + 0x4000 * banks];
        }
        else
